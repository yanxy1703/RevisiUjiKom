...
<div class="container">
    <div class="header">
        <h2>To-Do List</h2>
        <div class="user-menu">
            <button class="user-btn">Selamat Datang<?= htmlspecialchars($user['username']) ?></button>
        ...
        <?php while ($row = $result->fetch_assoc()): ?>
            <div class="task-item">
                ...
                <form method="POST" action="edit_task.php">
                    ...
                    <select name="status" onchange="return confirmTaskStatus(this);">
...
<script>
    ...
    function validateHistory(taskId) {
        const taskContainer = document.querySelector(`input[name="task_id"][value="${taskId}"]`).closest('.task-item');
        const checkboxes = taskContainer.querySelectorAll('.subtask-list input[type="checkbox"]:not(:checked)');
    ...
    function confirmTaskStatus(selectElement) {
        const selectedValue = selectElement.value;
        if (selectedValue === "Selesai") {
            const confirmed = confirm("Apakah Anda yakin tugas ini sudah selesai?");
            if (!confirmed) {
                selectElement.value = "Belum Selesai";
                return false;
            }
        }
        return true;
